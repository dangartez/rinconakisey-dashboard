...
import {
  SummaryIcon, AgendaIcon, UserCircleIcon, ServicesIcon, 
  ProfessionalsIcon, PromotionsIcon, NotificationsIcon, 
  SettingsIcon, LogoutIcon, StatisticsIcon,
  ChevronLeftIcon, ChevronRightIcon, CurrencyEuroIcon, CalculatorIcon, TicketIcon
} from '../icons/Icons';
...
const Sidebar: React.FC<SidebarProps> = ({ isCollapsed, toggleSidebar }) => {
  ...
  return (
    <aside className={`relative bg-slate-900 text-white flex flex-col p-4 h-screen overflow-y-hidden transition-all duration-300 ${isCollapsed ? 'w-20' : 'w-full md:w-64'}`}>
      ...
      <div className="flex-1 flex flex-col gap-2 overflow-y-auto custom-scrollbar">
        <nav className="flex-1 flex flex-col gap-2">
          ...
          <NavLink to="/clientes" className={getNavLinkClass}><UserCircleIcon className="h-5 w-5 flex-shrink-0" /><span className={`ml-3 whitespace-nowrap transition-opacity duration-200 ${isCollapsed ? 'opacity-0 hidden' : 'opacity-100'}`}>Clientes</span></NavLink>
          ...
          <NavLink to="/bonos" className={getNavLinkClass}><TicketIcon className="h-5 w-5 flex-shrink-0" /><span className={`ml-3 whitespace-nowrap transition-opacity duration-200 ${isCollapsed ? 'opacity-0 hidden' : 'opacity-100'}`}>Bonos</span></NavLink>
          ...
          <div className="border-t border-slate-700 my-2"></div> {/* Separador */}
